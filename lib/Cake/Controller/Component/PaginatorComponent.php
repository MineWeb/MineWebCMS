...
class PaginatorComponent extends Component
{
    ...
    public function paginate($object = null, $scope = [], $whitelist = [])
    {
        ...
        if (!$results) {
            $count = 0;
        } else if ($object->hasMethod('paginateCount')) {
            $count = $object->paginateCount($conditions, $recursive, $extra);
        } else if ($page === 1 && count($results) < $limit) {
            $count = count($results);
        } else {
    ...
    public function validateSort(Model $object, array $options, array $whitelist = [])
    {
        ...
        if (!empty($options['order']) && is_array($options['order'])) {
            ...
            foreach ($options['order'] as $key => $value) {
                ...
                } else if ($correctAlias && $object->hasField($key, true)) {
                    $order[$field] = $value;
                } else if (isset($object->{$alias}) && $object->{$alias}->hasField($field, true)) {
