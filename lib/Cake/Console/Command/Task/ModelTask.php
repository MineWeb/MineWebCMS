    ...
    public function fieldValidation($fieldName, $metaData, $primaryKey = 'id')
    {
        ...
        while ($anotherValidator === 'y') {
            ...
            if ($metaData['null'] != 1 && !in_array($fieldName, [$primaryKey, 'created', 'modified', 'updated'])) {
                if ($fieldName === 'email') {
                    $guess = $methods['email'];
                } else if ($metaData['type'] === 'string' && $metaData['length'] == 36) {
                    $guess = $methods['uuid'];
                ...
                } else if ($metaData['type'] === 'smallinteger') {
                    $guess = $methods['numeric'];
                } else if ($metaData['type'] === 'tinyinteger') {
                    $guess = $methods['numeric'];
    ...
    public function findHasOneAndMany(Model $model, $associations)
    {
        ...
        foreach ($this->_tables as $otherTable) {
            ...
            foreach ($tempFieldNames as $fieldName) {
                ...
                } else if ($otherTable === $model->table && $fieldName === 'parent_id') {
                    $assoc = [
