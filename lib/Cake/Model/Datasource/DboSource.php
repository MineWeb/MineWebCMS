...
class DboSource extends DataSource
{
    ...
    public function query()
    {
        ...
        } else if (count($args) > 1 && preg_match('/^find(\w*)By(.+)/', $args[0], $matches)) {
            $params = $args[1];
    ...
    public function value($data, $column = null, $null = true)
    {
        ...
        } else if (is_object($data) && isset($data->type, $data->value)) {
            if ($data->type === 'identifier') {
                return $this->name($data->value);
            } else if ($data->type === 'expression') {
                return $data->value;
            }
        } else if (in_array($data, ['{$__cakeID__$}', '{$__cakeForeignKey__$}'], true)) {
            return $data;
        }
    ...
    public function fullTableName($model, $quote = true, $schema = true)
    {
        ...
        } else if (!empty($this->config['prefix']) && strpos($model, $this->config['prefix']) !== 0) {
    ...
    public function fields(Model $Model, $alias = null, $fields = [], $quote = true)
    {
        ...
        if ($count >= 1 && !in_array($fields[0], ['*', 'COUNT(*)'])) {
            for ($i = 0; $i < $count; $i++) {
                ...
                } else if (preg_match('/^\(.*\)\s' . $this->alias . '.*/i', $fields[$i])) {
                    continue;
                } else if (!preg_match('/^.+\\(.*\\)/', $fields[$i])) {
                    $prepend = '';
                ...
                } else if (preg_match('/\(([\.\w]+)\)/', $fields[$i], $field)) {
                    if (isset($field[1])) {
    ...
    public function conditionKeysToString($conditions, $quoteValues = true, Model $Model = null)
    {
        ...
        foreach ($conditions as $key => $value) {
            ...
            if (is_numeric($key) && empty($value)) {
            ...
            } else if ((is_numeric($key) && is_array($value)) || in_array(strtolower(trim($key)), $this->_sqlBoolOps)) {
                ...
                } else if (is_array($value) && !empty($value) && !$valueInsert) {
                ...
                } else if (is_numeric($key) && !empty($value)) {
    ...
    public function order($keys, $direction = 'ASC', Model $Model = null)
    {
        ...
        while (!empty($keys)) {
            ...
            if (is_array($key)) {
                ...
                continue;
            } else if (is_object($key) && isset($key->type) && $key->type === 'expression') {
                $result[] = $key->value;
                continue;
            }
    ...
    public function prepareFields(Model $Model, $queryData)
    {
        ...
        } else if (!empty($Model->hasMany) && $Model->recursive > -1) {
        ...
        if ($type === 'belongsTo' || $type === 'hasOne') {
            if (isset($merge[$association])) {
                ...
                if (!isset($dataAssociation)) {
                    ...
                    } else if ($selfJoin && array_key_exists($association, $merge[0])) {
    ...
    protected function _prepareUpdateFields(Model $Model, $fields, $quoteValues = true, $alias = false)
    {
        ...
        foreach ($fields as $field => $value) {
            ...
            } else if (!$alias && strpos($field, '.') !== false) {
            ...
            } else if ($Model->getColumnType($field) === 'boolean' && (is_int($value) || is_bool($value))) {
    ...
    public function buildColumn($column)
    {
        ...
        } else if ($type !== 'timestamp' && !empty($column['null'])) {
            $out .= ' DEFAULT NULL';
        } else if ($type === 'timestamp' && !empty($column['null'])) {
            $out .= ' NULL';
