...
class CakeSchema extends CakeObject
{
    ...
    protected function _requireFile($path, $file)
    {
        if (file_exists($path . DS . $file) && is_file($path . DS . $file)) {
            ...
            return true;
        } else if (file_exists($path . DS . 'schema.php') && is_file($path . DS . 'schema.php')) {
            ...
            if (substr($value['type'], 0, 4) !== 'enum') {
                if (!isset($db->columns[$value['type']])) {
                    ...
                    if (isset($defaultCol['limit']) && $defaultCol['limit'] == $value['length']) {
                        unset($value['length']);
                    } else if (isset($defaultCol['length']) && $defaultCol['length'] == $value['length']) {
                        unset($value['length']);
                    }
                    unset($value['limit']);
                }
        ...
        foreach ($new as $name => $value) {
            if (isset($old[$name])) {
                ...
                if ($newUnique != $oldUnique) {
                    $diff = true;
                } else if (is_array($newColumn) && is_array($oldColumn)) {
                    $diff = ($newColumn !== $oldColumn);
                } else if (is_string($newColumn) && is_string($oldColumn)) {
                    $diff = ($newColumn != $oldColumn);
                } else {
                    $diff = true;
                }
