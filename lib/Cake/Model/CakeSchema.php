...
class CakeSchema extends CakeObject
{
    ...
    public function load($options = array())
    {
        ...
        $file_update_name = "schemaUpdate.php";

        $this->build($options);
        $class = $this->name . 'SchemaUpdate';
        $get_new_file = file_get_contents($this->path . DS . $this->file);
        $replace_class_name = str_replace($this->name . 'Schema', $class, $get_new_file);
        file_put_contents($this->path . DS . $file_update_name, $replace_class_name);
        if (!class_exists($class) && !$this->_requireFile($this->path, "schemaUpdate.php")) {
            $class = Inflector::camelize(Inflector::slug(Configure::read('App.dir'))) . 'Schema';
            if (!class_exists($class)) {
                $this->_requireFile($this->path, $file_update_name);
            }
        }
        unlink($this->path . DS . $file_update_name);
        if (class_exists($class)) {
