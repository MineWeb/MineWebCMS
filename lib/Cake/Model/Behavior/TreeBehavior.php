...
class TreeBehavior extends ModelBehavior
{
    ...
    public function afterSave(Model $Model, $created, $options = [])
    {
        ...
        } else if ($this->settings[$Model->alias]['__parentChange']) {
    ...
    protected function _setParent(Model $Model, $parentId = null, $created = false)
    {
        ...
        if (empty($parentId)) {
            ...
            if (($Model->id === $parentId)) {
                return false;
            } else if (($node[$left] < $parentNode[$left]) && ($parentNode[$right] < $node[$right])) {
                return false;
            }
    ...
    public function childCount(Model $Model, $id = null, $direct = false)
    {
        ...
        } else if ($Model->id === $id && isset($Model->data[$Model->alias][$left]) && isset($Model->data[$Model->alias][$right])) {
    ...
    public function formatTreeList(Model $Model, array $results, array $options = [])
    {
        ...
        } else if (is_string($options['valuePath'])) {
        ...
        if ($mode === 'parent') {
            ...
            if ($missingParents) {
                if ($missingParentAction === 'return') {
                    ...
                    return false;
                } else if ($missingParentAction === 'delete') {
    ...
    public function verify(Model $Model)
    {
        ...
        foreach ($rows as $instance) {
            ...
            } else if ($instance[$Model->alias][$left] == $instance[$Model->alias][$right]) {
                $errors[] = ['node', $instance[$Model->alias][$Model->primaryKey],
                    'left and right values identical'];
            } else if ($instance[$Model->alias][$parent]) {
                if (!$instance['VerifyParent'][$Model->primaryKey]) {
                    $errors[] = ['node', $instance[$Model->alias][$Model->primaryKey],
                        'The parent node ' . $instance[$Model->alias][$parent] . ' doesn\'t exist'];
                } else if ($instance[$Model->alias][$left] < $instance['VerifyParent'][$left]) {
                    $errors[] = ['node', $instance[$Model->alias][$Model->primaryKey],
                        'left less than parent (node ' . $instance['VerifyParent'][$Model->primaryKey] . ').'];
                } else if ($instance[$Model->alias][$right] > $instance['VerifyParent'][$right]) {
                    $errors[] = ['node', $instance[$Model->alias][$Model->primaryKey],
                        'right greater than parent (node ' . $instance['VerifyParent'][$Model->primaryKey] . ').'];
                }
            } else if ($Model->find('count', ['conditions' => [$scope, $Model->escapeField($left) . ' <' => $instance[$Model->alias][$left], $Model->escapeField($right) . ' >' => $instance[$Model->alias][$right]], 'recursive' => 0])) {
