...
// @codingStandardsIgnoreEnd
    ...
    public function setDataSource($dataSource = null)
    {
        ...
        } else if (isset($db->config['prefix'])) {
            $this->tablePrefix = $db->config['prefix'];
        }
    ...
    public function __isset($name)
    {
        ...
        foreach ($this->_associations as $type) {
            if (isset($name, $this->{$type}[$name])) {
                ...
                break;
            } else if (isset($name, $this->__backAssociation[$type][$name])) {
    ...
    protected function _doSave($data = null, $options = [])
    {
        ...
        if (!empty($options['fieldList'])) {
            ...
            } else if (Hash::dimensions($options['fieldList']) < 2) {
                $this->whitelist = $options['fieldList'];
            }
        } else if ($options['fieldList'] === null) {
            $this->whitelist = [];
        }
        ...
        foreach ($joined as $assoc => $data) {
            ...
            foreach ((array)$data as $row) {
                if ((is_string($row) && (strlen($row) === 36 || strlen($row) === 16)) || is_numeric($row)) {
                    ...
                    unset($values);
                } else if (isset($row[$habtm['associationForeignKey']])) {
                    if (!empty($row[$Model->primaryKey])) {
                        $newJoins[] = $row[$habtm['associationForeignKey']];
                    }

                    $newData[] = $row;
                } else if (isset($row[$join]) && isset($row[$join][$habtm['associationForeignKey']])) {
    ...
    protected function _findCount($state, $query, $results = [])
    {
        if ($state === 'before') {
            ...
            } else if (method_exists($db, 'expression') && is_string($query['fields']) && !preg_match('/count/i', $query['fields'])) {
                $query['fields'] = $db->calculate($this, 'count', [
    ...
    protected function _findNeighbors($state, $query, $results = [])
    {
        ...
        } else if (count($neighbors) === 1 && !$return['prev']) {
