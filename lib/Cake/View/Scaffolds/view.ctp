                ...
                foreach ($associations['belongsTo'] as $_alias => $_details) {
                    if ($_field === $_details['foreignKey']) {
                        ...
                        echo $this->Html->link(
                            ...
                            ['plugin' => $_details['plugin'], 'controller' => $_details['controller'], 'action' => 'view', ${$singularVar}[$_alias][$_details['primaryKey']]]
                        );
                        echo "\n\t\t&nbsp;</dd>\n";
                        break;
                    }
...
<div class="actions">
    ...
    <ul>
        <?php
        echo "\t\t<li>";
        echo $this->Html->link(__d('cake', 'Edit %s', $singularHumanName), ['action' => 'edit', ${$singularVar}[$modelClass][$primaryKey]]);
        echo " </li>\n";

        echo "\t\t<li>";
        echo $this->Form->postLink(__d('cake', 'Delete %s', $singularHumanName), ['action' => 'delete', ${$singularVar}[$modelClass][$primaryKey]], [], __d('cake', 'Are you sure you want to delete # %s?', ${$singularVar}[$modelClass][$primaryKey]));
        echo " </li>\n";

        echo "\t\t<li>";
        echo $this->Html->link(__d('cake', 'List %s', $pluralHumanName), ['action' => 'index']);
        echo " </li>\n";

        echo "\t\t<li>";
        echo $this->Html->link(__d('cake', 'New %s', $singularHumanName), ['action' => 'add']);
        echo " </li>\n";
                ...
                <ul>
                    <li><?php
                        echo $this->Html->link(
                            ...
                            ['plugin' => $_details['plugin'], 'controller' => $_details['controller'], 'action' => 'edit', ${$singularVar}[$_alias][$_details['primaryKey']]]
                        );
                        echo "</li>\n";
                        ?>
                </ul>
        ...
        <?php if (!empty(${$singularVar}[$_alias])): ?>
            <table cellpadding="0" cellspacing="0">
                ...
                foreach (${$singularVar}[$_alias] as ${$otherSingularVar}):
                    ...
                    echo $this->Html->link(
                        __d('cake', 'View'),
                        ['plugin' => $_details['plugin'], 'controller' => $_details['controller'], 'action' => 'view', ${$otherSingularVar}[$_details['primaryKey']]]
                    );
                    echo "\n";
                    echo "\t\t\t\t";
                    echo $this->Html->link(
                        __d('cake', 'Edit'),
                        ['plugin' => $_details['plugin'], 'controller' => $_details['controller'], 'action' => 'edit', ${$otherSingularVar}[$_details['primaryKey']]]
                    );
                    echo "\n";
                    echo "\t\t\t\t";
                    echo $this->Form->postLink(
                        __d('cake', 'Delete'),
                        ['plugin' => $_details['plugin'], 'controller' => $_details['controller'], 'action' => 'delete', ${$otherSingularVar}[$_details['primaryKey']]],
                        [],
