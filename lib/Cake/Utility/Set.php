    ...
    public static function extract($path, $data = null, $options = [])
    {
        ...
        do {
            ...
            foreach ($contexts as $key => $context) {
                ...
                } else if ($key === $token || (ctype_digit($token) && $key == $token) || $token === '.') {
                    $context['trace'][] = $key;
                    $matches[] = [
                        'trace' => $context['trace'],
                        'key' => $key,
    ...
    public static function matches($conditions, $data = [], $i = null, $length = null)
    {
        ...
        foreach ($conditions as $condition) {
            ...
            if (!preg_match('/(.+?)([><!]?[=]|[><])(.*)/', $condition, $match)) {
                ...
                } else if (preg_match_all('/(?:^[0-9]+|(?<=,)[0-9]+)/', $condition, $matches)) {
                    return in_array($i, $matches[0]);
                } else if (!array_key_exists($condition, $data)) {
                    return false;
                }
                continue;
            }
