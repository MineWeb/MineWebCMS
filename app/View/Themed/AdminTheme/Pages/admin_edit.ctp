<div class="row-fluid">
	...
	<div class="span12">
		...
		<div class="top-bar">
			<h3><i class="icon-cog"></i> <?= $Lang->get('EDIT_PAGE') ?></h3>
		</div>
		...
		<div class="well no-padding">
			...
			?>
				...
				<?php
				$this->Form->unlockField('Page.content');
				echo $this->Form->textarea('Page.content', array('value' => $page['content']));
				?>
				</div>
...
<script type="text/javascript">
    $("#news_add").submit(function( event ) {
        ...
        $.post("<?= $this->Html->url(array('controller' => 'pages', 'action' => 'edit_ajax', 'admin' => true)) ?>", { id : id, title : title, slug : slug, content : content }, function(data) {
          	data2 = data.split("|");
		  	if(data.indexOf('true') != -1) {
          		$('.ajax-msg').empty().html('<div class="alert alert-success" style="margin-top:10px;margin-right:10px;margin-left:10px;"><a class="close" data-dismiss="alert">×</a><i class="icon icon-exclamation"></i> <b><?= $Lang->get('SUCCESS') ?> :</b> '+data2[0]+'</i></div>').fadeIn(500);
          		 document.location.href="<?= $this->Html->url(array('controller' => 'pages', 'action' => 'admin_index', 'admin' => 'true')) ?>";
