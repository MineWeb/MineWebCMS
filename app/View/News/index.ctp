...
<div class="container">
    <div class="row">
        <div class="col-lg-8">
            ...
            <hr>
            <p>
                <span class="glyphicon glyphicon-time"></span> <?= $Lang->get('NEWS__POSTED_ON') . ' ' . $Lang->date($news['News']['created']); ?>
            </p>

            <hr>
            <p class="lead"><?= $news['News']['content'] ?></p>
            <button id="<?= $news['News']['id'] ?>" type="button"
                    class="btn btn-primary pull-right like<?= ($news['News']['liked']) ? ' active' : '' ?>"<?= (!$Permissions->can('LIKE_NEWS')) ? ' disabled' : '' ?>><?= $news['News']['count_likes'] ?>
                ...
                <div id="form-comment-fade-out">
                    ...
                    <div class="well">
                        ...
                        <form method="POST" data-ajax="true"
                              action="<?= $this->Html->url(['controller' => 'news', 'action' => 'add_comment']) ?>"
                ...
                <div class="media comment" id="comment-<?= $v['id'] ?>">
                    <a class="pull-left" href="#">
                        <img class="media-object"
                             src="<?= $this->Html->url(['controller' => 'API', 'action' => 'get_head_skin/']) ?>/<?= $v['author'] ?>/64"
                             alt="">
                    </a>
                    ...
                    <div class="col-lg-6">
                        <ul class="list-unstyled">
                            <?php foreach ($search_news as $k => $v) { ?>
                                <li>
                                    <a href="<?= $this->Html->url(['controller' => 'blog', 'action' => $v['News']['slug']]) ?>"><?= $v['News']['title'] ?></a>
                                </li>
                            <?php } ?>
                        </ul>
                    </div>
...
<script>
    function addcomment(data) {
        var d = new Date();
        var comment = '<div class="media"><a class="pull-left" href="#"><img class="media-object" src="<?= $this->Html->url(['controller' => 'API', 'action' => 'get_head_skin/']) ?>/<?= $user['pseudo'] ?>/64" alt=""></a><div class="media-body"><h4 class="media-heading"><?= $user['pseudo'] ?> <small>' + d.getHours() + 'h' + d.getMinutes() + '</small></h4>' + data['content'] + '</div></div>';
    ...
    function comment_delete(e) {
        ...
        $.post("<?= $this->Html->url(['controller' => 'news', 'action' => 'ajax_comment_delete']) ?>", inputs, function (data) {
            if (data == 'true') {
