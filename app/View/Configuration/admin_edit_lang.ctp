        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header with-border">
                    <h3 class="card-title"><?= $Lang->get('CONFIG__LANG_LABEL') ?></h3>
                </div>
                <div class="card-body">
                            ...
                            <?php if ($key != 'FOOTER_ADMIN') { ?>
                                <div class="form-group">
                                    ...
                                    <?php if ($key == "GLOBAL__FORMAT_DATE") { ?>
                                        <small><?= $Lang->get('CONFIG__LANG_AVAILABLE_VARIABLES') ?> : {%day}, {%month},
                                            {%year}, {%hour|24}, {%hour|12}, {%minutes}</small>
                                    <?php } ?>
                                    <?php if ($key == "SERVER__STATUS_MESSAGE") { ?>
                                        <small><?= $Lang->get('CONFIG__LANG_AVAILABLE_VARIABLES') ?> : {MOTD},
                                    ...
                                    <?php } ?>
                                    <?php if ($key == "RESET_PASSWORD_MAIL") { ?>
                                        <small><?= $Lang->get('CONFIG__LANG_AVAILABLE_VARIABLES') ?> : {EMAIL},
                                            {PSEUDO}, {LINK}.</small>
                                    <?php } ?>
