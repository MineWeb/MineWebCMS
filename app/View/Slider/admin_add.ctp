...
<div class="row-fluid">
	...
	<div class="span12">
		...
		<div class="top-bar">
			<h3><i class="icon-cog"></i> <?= $Lang->get('ADD_SLIDER') ?></h3>
		</div>
...
<script type="text/javascript">
    $("#slider_add").submit(function( event ) {
    	event.preventDefault();
        var $form = $( this );
        var title = $form.find("input[name='title']").val();
        var subtitle = $form.find("input[name='subtitle']").val();
        var url_img = $form.find("input[name='url_img']").val();
        $.post("<?= $this->Html->url(array('controller' => 'slider', 'action' => 'add_ajax', 'admin' => true)) ?>", { title : title, subtitle : subtitle, url_img : url_img }, function(data) {
          	data2 = data.split("|");
