                ...
                <div class="card-body">
                    <form method="post" data-ajax="true" data-upload-image="true" data-redirect-url="<?= $this->Html->url(['controller' => 'social', 'action' => 'index', 'admin' => 'true']) ?>" id="add-social-button">
                        ...
                        <div class="form-group">
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="radio" id="choose-is-fa" name="type" value="fa" <?php if($social_button['fa'] != null) { echo "checked"; } ?>>
                                <label class="form-check-label" for="choose-is-fa"><?= $Lang->get('SOCIAL__CHOOSE_TYPE_FA') ?></label>
                            </div>
                        </div>
                            ...
                            <div id="type-is-img" <?php if($social_button['img'] == null) { ?>class="d-none"<?php } ?>>
                                <div class="form-group mx-5">
                                    ...
                                    <input type="text" name="img" class="form-control img-or-fa-input" placeholder="https://images.google.com" value="<?= $social_button['img'] ?>">
                                </div>
                            ...
                            <div id="type-is-fa" <?php if($social_button['fa'] == null) { ?>class="d-none"<?php } ?>>
                                <div class="form-group mx-5">
                                    <label><?= $Lang->get('SOCIAL__BUTTON_FA') ?></label>
                                    <p><?= $Lang->get('SOCIAL_FA_DESC') ?><a target="_blank" href="https://fontawesome.com/" title="Lien vers fontawesome">https://fontawesome.com/</a>
                                    <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                            <span class="input-group-text">FA</span>
                                        </div>
                                        <input type="text" name="fa" class="form-control img-or-fa-input" placeholder="fab fa-teamspeak"  value="<?= $social_button['fa'] ?>">
                                    </div>
                                </div>
                                <div class="text-right mx-5">
                                    <a class="btn btn-default type-cancel"><?= $Lang->get('SOCIAL__CHOOSE_TYPE_CANCEL') ?></a>
...
<script>
    // Default select //
        ...
        function getSelectValue(value) {
            switch (value) {
                ...
                case '<?= strtolower($value['title']) ?>':
                    dispatchData('<?= $value['title'] ?>', '<?= $value['fa'] ?>', '<?= $value['img'] ?>', '<?= $value['color'] ?>');
                    break;
                <?php } ?>
                default:
                    globalReset();
        ...
        function dispatchData(title, fa, img, color) {
            $("#social-title").val(title);

            if(img.length > 0) {
                $('#choose-is-img').prop('checked', true);
                $("#type-is-img").removeClass("d-none");
                $("#type-is-fa").addClass("d-none");
                $("#type-is-img input").val(img);
            }
            if(fa.length > 0) {
                $('#choose-is-fa').prop('checked', true);
                $("#type-is-img").addClass("d-none");
                $("#type-is-fa").removeClass("d-none");
                $("#type-is-fa input").val(fa);
            }
