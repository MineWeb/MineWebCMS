...
class PagesController extends AppController
{
    ...
    public function display()
    {
        ...
        if (isset($passwd[1])) { // si il y a un truc en plus
            ...
            if (isset($passwd[0]) and $passwd[0] == "resetpasswd") { // si c'est pour reset le password
                if (!empty($passwd[1])) {
                    ...
                    $search = $this->Lostpassword->find('first', ['conditions' => ['key' => $passwd[1]]]); // on cherche la key de reset password
        ...
        $search_news = $this->News->find('all', ['recursive' => 1, 'limit' => '6', 'order' => 'id desc', 'conditions' => ['published' => 1]]); // on cherche les 3 dernières news (les plus veille)
    ...
    public function index($slug = false)
    {
        if ($slug != false) {
            $this->loadModel('Page');
            $search = $this->Page->find('all', ['conditions' => ['slug' => $slug]]);
            if (!empty($search)) {
                $this->layout = $this->Configuration->getKey('layout');
                $page = $search[0]['Page'];
        ...
        if ($this->isConnected and $this->Permissions->can('MANAGE_PAGE')) {
            if ($this->request->is('post')) {
                if (!empty($this->request->data['title']) and !empty($this->request->data['slug']) and !empty($this->request->data['content'])) {
                    ...
                    $this->response->body(json_encode(['statut' => true, 'msg' => $this->Lang->get('PAGE__ADD_SUCCESS')]));
    ...
    public function admin_edit($id = false)
    {
        if ($this->isConnected and $this->Permissions->can('MANAGE_PAGE')) {
            if ($id != false) {
                ...
                $this->layout = 'admin';
                $this->loadModel('Page');
                $page = $this->Page->find('all', ['conditions' => ['id' => $id]]);
                if (!empty($page)) {
                    $page = $page[0]['Page'];
                    $this->set(compact('page'));
                } else {
        ...
        if ($this->isConnected and $this->Permissions->can('MANAGE_PAGE')) {
            if ($this->request->is('post')) {
                if (!empty($this->request->data['id']) and !empty($this->request->data['title']) and !empty($this->request->data['slug']) and !empty($this->request->data['content'])) {
                    ...
                    $this->response->body(json_encode(['statut' => true, 'msg' => $this->Lang->get('PAGE__EDIT_SUCCESS')]));
