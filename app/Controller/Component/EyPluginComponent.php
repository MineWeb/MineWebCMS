            ...
            foreach ($pluginList as $key => $value) {
                ...
                if($lastVersion && $value->version != $lastVersion) {
                   $this->Lang = $this->controller->Lang;
                    return '<div class="alert alert-warning">' . $this->Lang->get('UPDATE__AVAILABLE') . ' ' . $this->Lang->get('UPDATE__PLUGIN') . ' <a href="' . Router::url(array('controller' => 'plugin', 'action' => 'index', 'admin' => true)) . '" style="margin-top: -6px;" class="btn btn-warning float-right">' . $this->Lang->get('GLOBAL__UPDATE_LOOK') . '</a></div>';
                }
            }
        }
    }
