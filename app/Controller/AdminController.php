...
class AdminController extends AppController
{
    ...
    function admin_index()
    {
        if ($this->isConnected and $this->Permissions->can('ACCESS_DASHBOARD')) {
            ...
            $nbr_comments = $this->Comment->find('count', ['conditions' => ['created LIKE' => date('Y-m-d') . '%']]);
            ...
            if ($this->EyPlugin->isInstalled('eywek.shop')) {
                ...
                $purchase = $this->ItemsBuyHistory->find('count', ['order' => 'id DESC']);
                $purchase_today = $this->ItemsBuyHistory->find('count', ['conditions' => ['created LIKE' => date('Y-m-d') . '%'], 'order' => 'id DESC']);
