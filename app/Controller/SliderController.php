...
class SliderController extends AppController
{
    ...
    public function admin_delete($id = false)
    {
        ...
        if ($this->isConnected and $this->Permissions->can('MANAGE_SLIDER')) {
            if ($id != false) {
                $this->loadModel('Slider');
                $find = $this->Slider->find('all', ['conditions' => ['id' => $id]]);
                if (!empty($find)) {
    ...
    public function admin_edit($id = false)
    {
        if ($this->isConnected and $this->Permissions->can('MANAGE_SLIDER')) {
            ...
            if ($id != false) {
                $this->loadModel('Slider');
                $search = $this->Slider->find('all', ['conditions' => ['id' => $id]]);
                if (!empty($search)) {
        ...
        if ($this->isConnected and $this->Permissions->can('MANAGE_SLIDER')) {
            ...
            if ($this->request->is('post')) {
                if (!empty($this->request->data['title']) and !empty($this->request->data['subtitle']) and !empty($this->request->data['id'])) {
                    ...
                    $this->response->body(json_encode(['statut' => true, 'msg' => $this->Lang->get('SLIDER__EDIT_SUCCESS')]));
        ...
        if ($this->isConnected and $this->Permissions->can('MANAGE_SLIDER')) {
            ...
            if ($this->request->is('post')) {
                ...
                if (!empty($this->request->data['title']) and !empty($this->request->data['subtitle'])) {
                    ...
                    $this->response->body(json_encode(['statut' => true, 'msg' => $this->Lang->get('SLIDER__ADD_SUCCESS')]));
                    $this->Session->setFlash($this->Lang->get('SLIDER__ADD_SUCCESS'), 'default.success');
                } else {
                    $this->response->body(json_encode(['statut' => false, 'msg' => $this->Lang->get('ERROR__FILL_ALL_FIELDS')]));
                }
            } else {
                $this->response->body(json_encode(['statut' => false, 'msg' => $this->Lang->get('NOT_POST', $language)]));
            }
        } else {
            throw new ForbiddenException();
        }
